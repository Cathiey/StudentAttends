...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $stmt = $conn->prepare("SELECT first_name FROM students WHERE email = ?");
        if ($stmt) {
        ...
        if (empty($first_name)) {
            ...
            // Extend OTP expiration time to 10 minutes
            $otp_expires_at = date("Y-m-d H:i:s", strtotime('+2 minutes'));

            // Store OTP and expiration in the database
            $stmt = $conn->prepare("UPDATE students SET otp = ?, otp_expires_at = ? WHERE email = ?");
            if ($stmt) {
