...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT otp, otp_expires_at FROM lecturers WHERE email = ?");
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($otp === $db_otp && strtotime($otp_expires_at) > time()) {
                ...
                header("Location: AFReset_Password.php?email=" . urlencode($email));
                exit;
            } else {
...
<body>
    <div class="otp-verification-container">
        <h2>Verify OTP</h2>
        <p>Email successfully sent!<br>Please enter the OTP that was sent to your email address.</p>
        ...
        <?php endif; ?>
        <p>Don't have an OTP? <a href="AFPassword.php">Request OTP here</a></p>
    </div>
</body>
</html>
